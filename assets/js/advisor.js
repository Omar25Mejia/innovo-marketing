(() => {
  ...
  const init = () => {
    if(!document.getElementById('innovoAdvisorStyle')) {
      const css = document.createElement('link'); css.id='innovoAdvisorStyle'; css.rel='stylesheet'; css.href='assets/css/advisor.css?v=20260905-1'; document.head.appendChild(css);
    }
    document.querySelectorAll('.nav-links a').forEach(a => {
      if (a.getAttribute('href') === 'soluciones.html') {
        a.href='contacto.html#franklin'; a.textContent=copy.nav; a.classList.remove('nav-cta');
      }
    });
    document.querySelectorAll('a[href*="wa.me/50375766631"]').forEach(a => {
      ...
      a.textContent='WhatsApp · '+DISPLAY;
    });
    const isHome = location.pathname.endsWith('index.html') || location.pathname.endsWith('/innovo-marketing/') || location.pathname.endsWith('/innovo-marketing');
    if(isHome) {
    ...
    if(location.pathname.endsWith('contacto.html')) {
      ...
      if(form && !form.dataset.franklinBound){
        ...
        form.addEventListener('submit', event => {
          ...
          const lines=lang==='en' ? ['Hello Franklin, I would like a quote from Innovo Marketing.','',`Name: ${data.get('name')||''}`,`Business: ${data.get('business')||''}`,`Email: ${data.get('email')||''}`,`Phone: ${data.get('phone')||''}`,`Business type: ${data.get('industry')||''}`,`What I need: ${data.get('package')||''}`,`Project: ${data.get('project')||''}`] : ['Hola Franklin, quiero solicitar una cotización de Innovo Marketing.','',`Nombre: ${data.get('name')||''}`,`Negocio: ${data.get('business')||''}`,`Correo: ${data.get('email')||''}`,`Teléfono: ${data.get('phone')||''}`,`Tipo de negocio: ${data.get('industry')||''}`,`Qué necesita: ${data.get('package')||''}`,`Proyecto: ${data.get('project')||''}`];
